...
describe('Currencies', () => {
  it('shows loading state', () => {
    ...
    renderWithRouter(<Currencies />);
    expect(screen.getByRole('progressbar')).toBeInTheDocument();
  });

  it('renders currency names in table', async () => {
    renderWithRouter(<Currencies />);
    await waitFor(() => {
  ...
  it('shows item count', async () => {
    renderWithRouter(<Currencies />);
    await waitFor(() => {
      expect(screen.getByText('3 currencies')).toBeInTheDocument();
    });
  });
  ...
  it('has correct column alignment', async () => {
    ...
    expect(screen.getByText('Currency').getAttribute('data-alignment')).toBe('left');
  });
