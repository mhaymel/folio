...
Swallowing the interrupt hides the cancellation signal from the rest of the call
stack and the thread pool.

**Bad:**

```java
final class QuoteFetcher {
    private final BlockingQueue<Quote> inbox;

    QuoteFetcher(BlockingQueue<Quote> inbox) {
        this.inbox = requireNonNull(inbox);
    }

    Quote awaitNext(Duration timeout) {
        try {
...
final class QuoteFetcher {
    ...
    Quote awaitNext(Duration timeout) {
        try {
            ...
            throw new FetchInterruptedException("poll interrupted", exception);
        }
    }
}
```
...
final class QuoteFetcher {
    ...
    Quote awaitNext(Duration timeout) throws InterruptedException {
        return inbox.poll(timeout.toMillis(), TimeUnit.MILLISECONDS);
    }
}
```
...
or a blocking primitive (`CountDownLatch.await`, `BlockingQueue.take`, 
`Future.get(timeout)`).
...
easily misused without the surrounding lock), and always have 
a higher-level replacement: `CountDownLatch`, `CyclicBarrier`, 
`Semaphore`, `BlockingQueue`, `CompletableFuture`, or `Phaser`.
...
final class JobCoordinator {
    ...
    private JobCoordinator(Object lock, boolean isReady) {
        this.lock = requireNonNull(lock);
...
unnecessary. Use `AtomicReference.updateAndGet` for instance-level 
lazy values.
