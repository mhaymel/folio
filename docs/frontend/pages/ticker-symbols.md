...
The UI should provide a view that displays all ISINs along with their associated ticker symbols and security names. This allows users to see the ticker symbol mappings for securities in their portfolio. The data should be fetched from the backend via a REST API endpoint that retrieves ISIN, ticker symbol, and name data from the database by joining the `isin`, `ticker_symbol`, `isin_ticker`, and `isin_name` tables.
...
Returns all ISIN to ticker symbol mappings with security names, sorted alphabetically by ISIN.
