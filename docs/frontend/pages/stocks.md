...
| Depot | Multi-select dropdown | Comma-separated exact match; options from `GET /api/stocks/filters` |
...
- **Row count:** When filters are active, display `"N of M stocks"` (e.g. `"12 of 50 stocks"`) where N = filtered count and M = total unfiltered count. When no filters are active, display `"M stocks"`. The `GET /api/stocks` response must include both `filteredCount` and `totalCount` to support this format.
